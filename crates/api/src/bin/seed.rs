...
use iam_core::{
    CalendarAction, CapabilityAction, ConnectionAction, MemoryAction, Org, OrgId, Permission, Principal, PrincipalId,
    PrincipalKind, Role, RoleId, Sensitivity, SpendAction, roles,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    let fixtures = [
        ...
        ("gym-speaker", "Basement Gym Speaker", PrincipalKind::Device, roles::DEVICE),
...
async fn ensure_roles(store: &dyn IdentityStore, org_id: OrgId) -> anyhow::Result<()> {
    for name in roles::ALL {
        let role = match store.get_role_by_name(org_id, name).await {
            Ok(r) => r,
            Err(_) => {
                let r = Role { id: RoleId::new(), org_id, name: name.to_string() };
                store.create_role(&r).await?;
                r
            }
        };
        // Always (re)assert the permission set so the fixture stays canonical.
        store.set_role_permissions(role.id, &permissions_for(name)).await?;
    }
    Ok(())
}
...
async fn ensure_principal(
    ...
    store
        .insert_codes(principal.id, CodePurpose::Registration, &[hash_code(&registration_token)?])
        .await?;

    println!("principal '{handle}' ({kind}) [{role_name}] id={}", principal.id);
    ...
    if kind == PrincipalKind::Human {
        ...
        let hashes: Vec<String> = recovery_codes.iter().map(|c| hash_code(c)).collect::<Result<_, _>>()?;
        store.insert_codes(principal.id, CodePurpose::Recovery, &hashes).await?;
